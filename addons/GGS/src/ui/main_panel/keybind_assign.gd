...
func _ready() -> void:
	Field.text = ggsManager.ggs_data["keybind_confirm_text"]
	Field.hint_tooltip = ""
	OptionLabel.hint_tooltip = "Popup message when using ggsKeybind components. Shown when asking for player input."


func _on_LineEdit_text_entered(new_text: String) -> void:
	Field.release_focus()
	
	ggsManager.ggs_data["keybind_confirm_text"] = new_text
